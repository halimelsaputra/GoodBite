...
const Checkout = () => {
  ...
  const [walletProvider, setWalletProvider] = useState("GoPay");
  ...
  useEffect(() => {
    const loadData = async () => {
      try {
        ...
        if (sellerUser) {
          toast.error("Anda sudah login sebagai penjual. Silakan logout terlebih dahulu untuk checkout.");
        ...
        const loadedPkg = await packageService.getPackageById(id);
        if (!loadedPkg) {
          toast.error("Paket tidak ditemukan");
          navigate("/jelajahi", { replace: true });
          return;
        }
        
        setPkg(loadedPkg);

        // Set customer data from session
  ...
  const handleNextStep = () => {
    if (currentStep === 1) {
      ...
      // Validate selected payment method inputs
      if (paymentMethod === "ewallet") {
        if (!walletNumber.trim()) {
          toast.error("Mohon isi nomor e-wallet");
          return;
        }
      }
      if (paymentMethod === "card") {
        if (!cardName.trim() || !cardNumber.trim() || !cardExpiry.trim() || !cardCvv.trim()) {
          toast.error("Mohon lengkapi data kartu");
          return;
        }
      }
      setCurrentStep(3);
  ...
  const paymentMethods = [
    ...
    { id: "ewallet", title: "E-Wallet", subtitle: "GoPay, OVO, Dana, ShopeePay", icon: Smartphone },
    { id: "card", title: "Kartu Kredit/Debit", subtitle: "Visa, Mastercard", icon: CreditCard }
  ];

  const getPaymentLabel = () => {
    if (paymentMethod === "cash") return "Bayar di Tempat";
    if (paymentMethod === "ewallet") return `E-Wallet (${walletProvider})`;
    if (paymentMethod === "card") return "Kartu Kredit/Debit";
                      ...
                      <CardContent className="space-y-4">
                        ...
                        {paymentMethod === "ewallet" && (
                          <div className="space-y-3 border rounded-xl p-4">
                            ...
                            <p className="text-sm text-muted-foreground">
                              Pastikan nomor e-wallet aktif untuk menerima permintaan pembayaran.
                            </p>
                          </div>
                        )}
                        ...
                        {paymentMethod === "card" && (
                          <div className="space-y-3 border rounded-xl p-4">
                            ...
                            <p className="text-sm text-muted-foreground">
                              Data kartu hanya digunakan untuk simulasi pembayaran di aplikasi ini.
                            </p>
                          </div>
                        )}
                        ...
                        <div className="bg-green-50 border border-green-200 rounded-xl p-4">
                          <p className="text-sm text-green-800">
                             Pembayaran aman dan mudah. Periksa kembali data sebelum lanjut.
                          </p>
                        </div>
                      </CardContent>
                    </Card>
                        ...
                        <div className="space-y-4">
                          ...
                          <div className="bg-gray-50 rounded-xl p-4">
                            <div className="flex items-center gap-3">
                              <Wallet className="h-5 w-5 text-primary" />
                              <div>
                                <span className="font-medium block">{getPaymentLabel()}</span>
                                {paymentMethod === "cash" && (
                                  <span className="text-xs text-muted-foreground">Bayar saat ambil paket</span>
                                )}
                                {paymentMethod === "ewallet" && (
                                  <span className="text-xs text-muted-foreground">
                                    Kirim permintaan ke {walletProvider} • {walletNumber || "-"}
                                  </span>
                                )}
                                {paymentMethod === "card" && (
                                  <span className="text-xs text-muted-foreground">
                                    {cardName || "Nama kartu"} • {cardNumber || "**** **** **** ****"}
                                  </span>
                                )}
                              </div>
                            </div>
