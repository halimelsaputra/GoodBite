...
const PesananDetail = () => {
  ...
  useEffect(() => {
    ...
    if (sellerUser) {
      toast.error("Anda sudah login sebagai penjual. Silakan logout terlebih dahulu untuk akses detail pesanan.");
  ...
  const handleRatingSubmit = () => {
    ...
    const nameToSend = customerName || "Pengguna GoodBite";
    if (!comment.trim()) {
      toast.error("Isi ulasan terlebih dahulu");
      return;
    }
    ...
    const submitReview = async () => {
      try {
        ...
        await AuthService.createPackageReview(order.packageId, {
          name: nameToSend,
          rating,
          comment,
        });
        toast.success("Terima kasih atas review kamu!");
        setShowRating(false);
        setComment("");
        setRating(0);
      } catch (error) {
                    ...
                    <Card className="border-2 border-accent">
                      <CardContent className="p-8 space-y-6">
                        <div className="text-center space-y-2">
                          <h3 className="font-bold text-2xl">Beri Review</h3>
                          <p className="text-muted-foreground">
