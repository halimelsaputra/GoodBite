...
const SellerRegister = () => {
  ...
  useEffect(() => {
    ...
    if (customerUser) {
      toast.error("Anda sudah login sebagai pembeli. Silakan logout terlebih dahulu untuk mendaftar sebagai penjual.");
  ...
  const validatePhone = (phoneValue: string) => /^(08|62)[0-9]{9,11}$/.test(phoneValue.replace(/\D/g, ""));

  const handleRegister = async (e: React.FormEvent) => {
    e.preventDefault();
    setIsLoading(true);

    if (!username || !storeName || !email || !phone || !password || !confirmPassword) {
    ...
    try {
      ...
      toast.success("Akun penjual berhasil dibuat!");
      navigate("/dashboard-penjual");
    } catch (error: any) {
      toast.error(error.message || "Gagal mendaftar sebagai penjual");
    } finally {
      setIsLoading(false);
    }
  };
      ...
      <main className="flex-1 flex items-center justify-center py-16 md:py-24 relative overflow-hidden">
        ...
        <div className="container max-w-lg relative z-10">
          <div className="shadow-input mx-auto w-full rounded-none bg-white p-4 md:rounded-2xl md:p-8 dark:bg-black">
            <h2 className="text-xl font-bold text-neutral-800 dark:text-neutral-200">Daftar Penjual</h2>
            <p className="mt-2 max-w-sm text-sm text-neutral-600 dark:text-neutral-300">
              Buat akun penjual untuk mulai menjual paket Anda.
            </p>
            ...
            <form onSubmit={handleRegister} className="my-8 space-y-4">
              ...
              <div className="pt-4 border-t">
                <p className="text-center text-sm text-muted-foreground">
                  Sudah punya akun?{" "}
                  <Link to="/seller-login" className="text-black font-semibold hover:underline">
                    Masuk di sini
                  </Link>
                </p>
              </div>
