...
export class OrderService {
  ...
  /**
   * Create a new order and reduce package stock
   */
  public async createOrder(orderData: OrderData): Promise<Order> {
    ...
    // Reduce package stock for seller packages (API + local cache)
    await this.reducePackageStock(orderData.packageId);
    
    return newOrder;
  }

  /**
   * Reduce package stock from seller packages (API-first, falls back silently)
   */
  private async reducePackageStock(packageId: string): Promise<void> {
    try {
      ...
      pkg.decreaseAvailability();

      // Persist to backend if seller package
      if (pkg.sellerId) {
        const newAvailable = Math.max(pkg.available, 0);
        await AuthService.updatePackage(pkg.sellerId, pkg.id, { available: newAvailable });
      }
    } catch (error) {
      console.error("Error reducing package stock:", error);
    }
  }
